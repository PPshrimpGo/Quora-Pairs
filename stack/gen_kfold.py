...
train['test_id'] = -1
test['is_duplicate'] = 0
col = [c for c in train.columns if (c[:1] == 'z' or c[:1] == 'f')] + ['test_id','is_duplicate']
...
train['uid'] = np.arange(train.shape[0])
test['uid'] = np.arange(test.shape[0])

train.to_csv('train_fold_all.csv',index=False)
test.to_csv('test_fold_all.csv',index=False)

sp = int(train.shape[0]/5)

train1 = train[sp:]
test1 = pd.concat([train[0:sp],test])

train1.to_csv('train_fold_1.csv',index = False)
test1.to_csv('test_fold_1.csv',index = False)
print '1'

train2 = pd.concat([train[0:sp],train[2*sp:]])
test2 = pd.concat([train[sp:2*sp],test])
train2.to_csv('train_fold_2.csv',index = False)
test2.to_csv('test_fold_2.csv',index = False)

print '2'
train3 = pd.concat([train[0:2*sp],train[3*sp:]])
test3 = pd.concat([train[2*sp:3*sp],test])
train3.to_csv('train_fold_3.csv',index = False)
test3.to_csv('test_fold_3.csv',index = False)

print '3'
train4 = pd.concat([train[0:3*sp],train[4*sp:]])
test4 = pd.concat([train[3*sp:4*sp],test])
train4.to_csv('train_fold_4.csv',index = False)
test4.to_csv('test_fold_4.csv',index = False)

print '4'
train5 = train[0:4*sp]
test5 = pd.concat([train[4*sp:],test])
train5.to_csv('train_fold_5.csv',index = False)
test5.to_csv('test_fold_5.csv',index = False)
print "done!"
