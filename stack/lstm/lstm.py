...
TRAIN_DATA_FILE = '../kfold/train_fold_all_lstm.csv'
TEST_DATA_FILE = '../kfold/test_fold_all_lstm.csv'

TRAIN_FEATURE_FILE = '../kfold/train_fold_all.csv'
TEST_FEATURE_FILE = '../kfold/test_fold_all.csv'
...
leaks = train_df[col]
test_leaks = test_df[col]

leaks = leaks.fillna(0.0)
test_leaks = test_leaks.fillna(0.0)
...
submission = pd.DataFrame({'test_id':test_ids, 'LSTM_predictions':preds.ravel()})
submission.to_csv('LSTM_%.4f_'%(bst_val_score)+STAMP+'_all.csv', index=False)